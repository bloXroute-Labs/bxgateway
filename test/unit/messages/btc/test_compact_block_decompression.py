from bxcommon.services.transaction_service import TransactionService
from bxcommon.test_utils.abstract_test_case import AbstractTestCase
from bxcommon.test_utils.mocks.mock_node import MockNode
from bxcommon.utils import convert, crypto
from bxgateway.btc_constants import BTC_SHA_HASH_LEN
from bxgateway.messages.btc.block_btc_message import BlockBtcMessage
from bxgateway.messages.btc.compact_block_btc_message import CompactBlockBtcMessage
from bxgateway.messages.btc.compact_block_decompression import decompress_compact_block, \
    decompress_recovered_compact_block
from bxgateway.utils.btc.btc_object_hash import BtcObjectHash


class TestCompactBlockParser(AbstractTestCase):
    COMPACT_BLOCK_BYTES_HEX = ""
    FULL_BLOCK_BYTES_HEX = ""
    MAGIC = 4206867930
    BLOCK_TRANSACTIONS_COUNT = 11

    def test_decompress_compact_block(self):
        full_block_msg = BlockBtcMessage(
            buf=bytearray(convert.hex_to_bytes(self.FULL_BLOCK_BYTES_HEX))
        )
        self.assertIsNotNone(full_block_msg)
        self.assertEqual(self.BLOCK_TRANSACTIONS_COUNT, full_block_msg.txn_count())

        transaction_service = TransactionService(MockNode("127.0.0.1", 123), 0)

        for tx in full_block_msg.txns():
            tx_hash = BtcObjectHash(crypto.bitcoin_hash(tx), length=BTC_SHA_HASH_LEN)
            transaction_service.set_transaction_contents(tx_hash, tx)

        self.assertEqual(len(full_block_msg.txns()),
                         len(list(transaction_service.iter_transaction_hashes())))

        compact_block_msg = CompactBlockBtcMessage(
            buf=bytearray(convert.hex_to_bytes(self.COMPACT_BLOCK_BYTES_HEX))
        )
        parse_result = decompress_compact_block(self.MAGIC, compact_block_msg, transaction_service)

        self.assertTrue(parse_result.success)
        self.assertIsInstance(parse_result.block_btc_message, BlockBtcMessage)
        self.assertEqual(len(full_block_msg.rawbytes()), len(parse_result.block_btc_message.rawbytes()))
        self.assertEqual(full_block_msg.rawbytes().tobytes(), parse_result.block_btc_message.rawbytes().tobytes())

    def test_decompress_recovered_compact_block(self):
        full_block_msg = BlockBtcMessage(
            buf=bytearray(convert.hex_to_bytes(self.FULL_BLOCK_BYTES_HEX))
        )
        self.assertIsNotNone(full_block_msg)
        self.assertEqual(11, full_block_msg.txn_count())

        transaction_service = TransactionService(MockNode("127.0.0.1", 123), 0)

        recovered_transactions = []

        index = 0
        for tx in full_block_msg.txns():
            if index % 2 == 0:
                tx_hash = BtcObjectHash(crypto.bitcoin_hash(tx), length=BTC_SHA_HASH_LEN)
                transaction_service.set_transaction_contents(tx_hash, tx)
            else:
                recovered_transactions.append(tx)
            index += 1

        compact_block_msg = CompactBlockBtcMessage(
            buf=bytearray(convert.hex_to_bytes(self.COMPACT_BLOCK_BYTES_HEX))
        )
        parse_result = decompress_compact_block(self.MAGIC, compact_block_msg, transaction_service)

        self.assertFalse(parse_result.success)
        self.assertIsNone(parse_result.block_btc_message)
        self.assertEqual(len(recovered_transactions), len(parse_result.missing_transactions_indices))
        self.assertEqual(self.BLOCK_TRANSACTIONS_COUNT, len(parse_result.block_transactions))
        self.assertEqual(len(recovered_transactions), len(parse_result.missing_transactions_indices))

        for missing_index in parse_result.missing_transactions_indices:
            self.assertNotEqual(0, missing_index % 2)

        recovery_result = decompress_recovered_compact_block(self.MAGIC, compact_block_msg,
                                                             parse_result.block_transactions,
                                                             parse_result.missing_transactions_indices,
                                                             recovered_transactions)
        self.assertTrue(recovery_result.success)
        self.assertIsInstance(recovery_result.block_btc_message, BlockBtcMessage)
        self.assertEqual(len(full_block_msg.rawbytes()), len(recovery_result.block_btc_message.rawbytes()))
        self.assertEqual(full_block_msg.rawbytes().tobytes(), recovery_result.block_btc_message.rawbytes().tobytes())
