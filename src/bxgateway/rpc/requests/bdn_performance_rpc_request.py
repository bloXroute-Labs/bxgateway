...
class BdnPerformanceRpcRequest(AbstractRpcRequest):
    ...
    def _calc_bdn_performance_stats(self) -> Dict[str, Any]:
        stats = {}
        interval_data: Optional[GatewayBdnPerformanceStatInterval] = \
        ...
        stats["interval_start_time"] = str(datetime.fromtimestamp(interval_data.start_time))
        stats["interval_end_time"] = str(datetime.fromtimestamp(interval_data.end_time))


        if interval_data.new_blocks_received_from_bdn + interval_data.new_blocks_received_from_blockchain_node == 0:
            stats[BLOCKS_FROM_BDN] = float("nan")
        if interval_data.new_tx_received_from_bdn + interval_data.new_tx_received_from_blockchain_node == 0:
            stats[TX_FROM_BDN] = float("nan")

        if BLOCKS_FROM_BDN not in stats:
            stats[BLOCKS_FROM_BDN] = stats_format.percentage(100 * (interval_data.new_blocks_received_from_bdn /
        ...
        if TX_FROM_BDN not in stats:
            stats[TX_FROM_BDN] = stats_format.percentage(100 * (interval_data.new_tx_received_from_bdn /
