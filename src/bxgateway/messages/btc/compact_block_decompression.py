...
def decompress_compact_block(
    ...
    pre_filled_txs = msg.pre_filled_transactions()
    pre_filled_tx_count = len(pre_filled_txs)
    short_ids_count = len(short_ids)
    total_txs_count = pre_filled_tx_count + short_ids_count
    ...
    for index in range(total_txs_count):
        if index not in pre_filled_txs:
            ...
            block_transactions.append(pre_filled_tx)
            size += len(pre_filled_tx)

    if len(missing_transactions_indices) > 0:
        return CompactBlockDecompressionResult(
            False,
            None,
            short_ids_count,
            pre_filled_tx_count,
    ...
    return CompactBlockDecompressionResult(
        True,
        BlockBtcMessage(buf=block_msg_bytes),
        short_ids_count,
        pre_filled_tx_count,
        None,
        None
...
def decompress_recovered_compact_block(
    ...
    return CompactBlockRecoveryResult(
        True,
        BlockBtcMessage(buf=block_msg_bytes)
    )
